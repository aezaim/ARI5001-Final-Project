...
draw_initial_graph(G, 'O', positions)
annotate = {node: {'dijkstra': float('inf'), 'a_star': float('inf')} for node in G.nodes()}
dijkstra_came_from = dijkstra(G, 'A', 'O', positions, annotate)
a_star_came_from = a_star(G, 'A', 'O', positions, annotate)
...
path_dijkstra = reconstruct_path(dijkstra_came_from, 'A', 'O')
path_a_star = reconstruct_path(a_star_came_from, 'A', 'O')
